...
exports.addEvent = async (
  ...
  const newEvent = {
    ...
    coordinate_fuzzy: {
      type: "Point",
      coordinates: [Number(longitude_fuzzy), Number(latitude_fuzzy)],
    },
    event_city,
    event_description,
...
exports.updateEvent = (_id, patchBody) => {
  ...
  if (
    !patchBody.event_name.match(letterRegex) ||
    !patchBody.event_date.match(dateRegex) ||
    isNaN(patchBody.event_duration) ||
    !Array.isArray(patchBody.attendees)
  ) {
    return Promise.reject({ status: 400, msg: "Bad Request" });
  }
  ...
  if (patchBody.event_date)
    updateObj.event_date = new Date(patchBody.event_date);
  if (patchBody.event_description)
    updateObj.event_description = patchBody.event_description;
  if (patchBody.event_duration)
    updateObj.event_duration = Number(patchBody.event_duration);
  if (patchBody.attendees) {
    updateObj.attendees = patchBody.attendees;
    updateObj.spaces_free =
      patchBody.max_attendees - patchBody.attendees.length;
  }

  return connectToDatabase().then((client) => {
    const eventsCollection = client.db("condiviso").collection("events");
    return eventsCollection
      .updateOne({ _id: _id }, { $set: updateObj })
      .then(() => {
